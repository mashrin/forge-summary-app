...
function mergedRanges(first, second) {
    ...
    if (i < firstLength) {
        newRanges.push.apply(newRanges, tslib_1.__spread(first.slice(i)));
    }
    if (j < secondLength) {
        newRanges.push.apply(newRanges, tslib_1.__spread(second.slice(i)));
    }
    return newRanges;
}
function increaseRowSpan(tr, rect, row) {
    ...
    for (var col = 0; col < map.width; col++) {
        ...
        tr.setNodeMarkup(mappedPos, undefined, tslib_1.__assign(tslib_1.__assign({}, attrs), { rowspan: attrs.rowspan + 1 }));
        col += attrs.colspan - 1;
    }
}
...
function isLastCellInRow(rect, row, col) {
    ...
    return rowNode.childCount === 1 && !table_map_1.hasMergedColumns(rect.map, row, col);
}
        ...
        try {
            for (var _b = tslib_1.__values(this.rows), _c = _b.next(); !_c.done; _c = _b.next()) {
                ...
                if (isDelete) {
                    ...
                    invertedRows.push(tslib_1.__assign(tslib_1.__assign({}, row), { from: row.from + offset, to: row.from + offset }));
                }
                else {
